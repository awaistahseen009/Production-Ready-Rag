...
def evaluate_answers(logger):
    ...
    total_faith = total_ctx_rel = total_ctx_util = total_unsupp = 0.0
...
def save_plots_retrieval(df: pd.DataFrame, run_dir: Path):
    ...
    bars = plt.bar(df["category"], df["avg_mrr"], edgecolor="black")
    ...
    for bar in bars:
        h = bar.get_height()
        plt.text(bar.get_x() + bar.get_width() / 2, h + 0.01, f"{h:.1%}",
...
def save_plots_answers(df: pd.DataFrame, run_dir: Path):
    ...
    for metric in metrics:
        ...
        plt.figure(figsize=(12, 6))
        bars = plt.bar(df["category"], df[metric], edgecolor="black")
        ...
        for bar in bars:
            h = bar.get_height()
            plt.text(bar.get_x() + bar.get_width() / 2, h + 0.05, f"{h:.2f}",
                     ha="center", va="bottom", fontsize=10, fontweight="bold")
...
def main():
    parser = argparse.ArgumentParser(
        description="Run RAG evaluation and save results + graphs"
    )
    parser.add_argument(
        "--run_name",
        type=str,
        required=True,
        help="Name of this run (used for output folder)"
    )
    args = parser.parse_args()

    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")

    # 🔹 ONLY CHANGE: save inside this file's directory
